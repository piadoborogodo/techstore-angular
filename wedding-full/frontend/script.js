...
let reserved = {};   // { gift_id: reserved_by }
...
async function fetchReservations() {
  try {
    ...
    reserved = {};
    data.forEach(r => { reserved[r.gift_id] = r.reserved_by; });
    render();
  } catch (err) {
...
async function postReservation(giftId, name) {
  const res = await fetch(`${API_URL}/reservations`, {
    method: 'POST',
...
async function confirmReserve() {
  ...
  try {
    await postReservation(pendingId, name);
    closeModal();
